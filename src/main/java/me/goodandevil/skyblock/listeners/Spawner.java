...
public class Spawner implements Listener {
	...
	public void onSpawnSpawn(SpawnerSpawnEvent event) {
		...
		if (location.getWorld().getName().equals(skyblock.getWorldManager().getWorld(Location.World.Normal).getName())
						...
						if (upgrades != null && upgrades.size() > 0 && upgrades.get(0).isEnabled()
							...
							if (NMSUtil.getVersionNumber() > 12) {
								...
								try {
									...
									try {
										Field TileEntityMobSpawnerField = spawner.getClass()
												.getDeclaredField("spawner");
